...
class NLCUnet_pre(nn.Module):
    def __init__(
        ...
        super(NLCUnet_pre, self).__init__()

        self.ksize = kernel_size
        self.scale = upscale

        self.Restorer = Restorer(nf=nf, nb=nb, scale=self.scale, input_para=input_para)
        weights_dict2 = torch.load("/share/home/104825/jiancong/NLCUnet/experiments/NLCUnet/NLCUnetx4_setting1_good/models/330000_G.pth")
        weights_dict1 = torch.load("/share/home/104825/jiancong/685000_G.pth")
        # 加载官方模型参数到模型中
        self.Restorer.load_state_dict(weights_dict1, strict=False)
        self.Restorer.load_state_dict(weights_dict2, strict=False)
