...
class ENLABlock(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, average=True, conv=default_conv, height=None, weight=None):
        ...
        self.ddwc9x9_shuff1 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=9, groups=channel, padding=((9//2)*4), dilation=4)
        self.dwc7x7_shuff1 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=7, groups=channel, padding=7 // 2)

        self.ddwc9x9_shuff2 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=9, groups=channel, padding=((9//2)*4), dilation=4)
        self.dwc7x7_shuff2 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=7, groups=channel, padding=7 // 2)

        self.ddwc9x9_shuff3 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=9, groups=channel, padding=((9//2)*4), dilation=4)
        self.dwc7x7_shuff3 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=7, groups=channel, padding=7 // 2)

        self.invariable1 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.invariable2 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.invariable3 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        # 定义分组为2的卷积层
        self.fusion_ddwc_dwc_1 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1, padding=0, groups=2)

        self.fusion_ddwc_dwc_2 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1, padding=0, groups=2)

        self.fusion_ddwc_dwc_3 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1, padding=0, groups=2)

        self.fusion_ddwc_dwc = nn.Conv2d(in_channels=3 * channel, out_channels=channel, kernel_size=1, stride=1, padding=0, groups=2)
    ...
    def forward(self, x):
        ...
        out = out1 + out2 + v_dwc + x_ft1
        
        # 获取张量最后一个维度的大小
        out_view = out.view(N, C, H*W)
        dim = out_view.size(-1)

        # 生成三组不同的随机排列索引
        indices1 = torch.randperm(dim)
        indices2 = torch.randperm(dim)
        indices3 = torch.randperm(dim)

        # 将索引应用到最后一个维度上进行三次打乱
        out_shuffled1 = out_view[..., indices1]
        out_shuffled2 = out_view[..., indices2]
        out_shuffled3 = out_view[..., indices3]

        # 重新将打乱后的张量转换为原始形状
        out_shuffled1 = out_shuffled1.view((N, C, H, W))
        out_shuffled2 = out_shuffled2.view((N, C, H, W))
        out_shuffled3 = out_shuffled3.view((N, C, H, W))

        ddwc9x9_shuffled1 = self.ddwc9x9_shuff1(out_shuffled1)
        dwc7x7_shuffled1 = self.dwc7x7_shuff1(out_shuffled1)

        ddwc9x9_shuffled2 = self.ddwc9x9_shuff2(out_shuffled2)
        dwc7x7_shuffled2 = self.dwc7x7_shuff2(out_shuffled2)

        ddwc9x9_shuffled3 = self.ddwc9x9_shuff3(out_shuffled3)
        dwc7x7_shuffled3 = self.dwc7x7_shuff3(out_shuffled3)
        
        # 创建一个空的 cat_shuffled 特征图，形状为 (batch_size, channels, height, width)
        cat_shuffled_1 = torch.zeros(N, 2 * C, H, W).cuda()
        cat_shuffled_2 = torch.zeros(N, 2 * C, H, W).cuda()
        cat_shuffled_3 = torch.zeros(N, 2 * C, H, W).cuda()

        # 将 ddwc9x9_shuffled 的全部通道赋值给 cat_shuffled1 的奇数通道
        cat_shuffled_1[:, ::2, :, :] = ddwc9x9_shuffled1
        # 将 dwc7x7_shuffled 的全部通道赋值给 cat_shuffled1 的偶数通道
        cat_shuffled_1[:, 1::2, :, :] = dwc7x7_shuffled1

        # 将 ddwc9x9_shuffled 的全部通道赋值给 cat_shuffled2 的奇数通道
        cat_shuffled_2[:, ::2, :, :] = ddwc9x9_shuffled2
        # 将 dwc7x7_shuffled 的全部通道赋值给 cat_shuffled2 的偶数通道
        cat_shuffled_2[:, 1::2, :, :] = dwc7x7_shuffled2

        # 将 ddwc9x9_shuffled 的全部通道赋值给 cat_shuffled3 的奇数通道
        cat_shuffled_3[:, ::2, :, :] = ddwc9x9_shuffled3
        # 将 dwc7x7_shuffled 的全部通道赋值给 cat_shuffled3 的偶数通道
        cat_shuffled_3[:, 1::2, :, :] = dwc7x7_shuffled3

        cat_shuffled_1 = self.fusion_ddwc_dwc_1(cat_shuffled_1)
        cat_shuffled_2 = self.fusion_ddwc_dwc_2(cat_shuffled_2)
        cat_shuffled_3 = self.fusion_ddwc_dwc_3(cat_shuffled_3)

        out_avg = self.avg(out)
        
        out_iv1 = self.invariable1(out_avg)
        cat_shuffled_1 = cat_shuffled_1 * out_iv1
        
        out_iv2 = self.invariable2(out_avg)
        cat_shuffled_2 = cat_shuffled_2 * out_iv2
        
        out_iv3 = self.invariable3(out_avg)
        cat_shuffled_3 = cat_shuffled_3 * out_iv3

        cat_shuffled = [cat_shuffled_1, cat_shuffled_2, cat_shuffled_3]
        cat_shuffled = torch.cat(cat_shuffled, 1)
        cat_shuffled = self.fusion_ddwc_dwc(cat_shuffled)
        
        out_dwc_3x3 = self.dwc_3x3(out)
        out_dwc_5x5 = self.dwc_5x5(out)
        out_dwc_7x7 = self.dwc_7x7(out)

        dwc_out = [cat_shuffled, out_dwc_3x3, out_dwc_5x5, out_dwc_7x7]
...
#
#        # conv1x1_2
#        self.conv1x1_2 = nn.Conv2d(nf * 2, nf * 2, 1, 1)
#
#        # decoder block 1
...
#        f_conv1 = self.ca1(self.conv1x1_1(torch.cat([f_en2, f_latent_up], dim=1)))
#        f_de1 = self.decoder_block1(f_conv1)
#
#        f_de1_up = self.up2(f_de1)
#        f_cat2 = self.ca2(self.conv1x1_2(torch.cat([f_en1, f_de1_up], dim=1)))
