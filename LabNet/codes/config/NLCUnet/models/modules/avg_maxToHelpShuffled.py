...
class ENLABlock(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, average=True, conv=default_conv, height=None, weight=None):
        ...
        self.avg_invariable1 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.max_invariable1 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.avg_invariable2 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.max_invariable2 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.avg_invariable3 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
        ...
        self.max_invariable3 = nn.Sequential(
            nn.Conv2d(channel, channel // 16, 1, 1, 0),
            nn.GELU(),
            nn.Conv2d(channel // 16, channel, 1, 1, 0),
            nn.Sigmoid()
...
#        self.dwc_5x5 = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=5, groups=channel, padding=5 // 2)
        ...
        self.fusion_ddwc_dwc_1 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1)

        self.fusion_ddwc_dwc_2 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1)

        self.fusion_ddwc_dwc_3 = nn.Conv2d(in_channels=2 * channel, out_channels=channel, kernel_size=1, stride=1)

        self.fusion_ddwc_dwc = nn.Conv2d(in_channels=3 * channel, out_channels=channel, kernel_size=1, stride=1)
...
#        self.w_final = nn.Conv2d(4 * channel, channel, 1)
        ...
        self.w_final = nn.Conv2d(2 * channel, channel, 1)
    ...
    def forward(self, x):
        ...
        out_avg = self.avg_pool(out)
        out_max = self.max_pool(out)
        
        out_avg1 = self.avg_invariable1(out_avg)
        out_max1 = self.max_invariable1(out_max)
        
        out_avg2 = self.avg_invariable2(out_avg)
        out_max2 = self.max_invariable2(out_max)
        
        out_avg3 = self.avg_invariable3(out_avg)
        out_max3 = self.max_invariable3(out_max)

        ddwc9x9_shuffled1 = self.ddwc9x9_shuff1(out_shuffled1)
        ddwc9x9_shuffled1_avg1 = ddwc9x9_shuffled1 * out_avg1
        ddwc9x9_shuffled1_max1 = ddwc9x9_shuffled1 * out_max1
        cat_shuffled1 = [ddwc9x9_shuffled1_avg1, ddwc9x9_shuffled1_max1]
        cat_shuffled1 = torch.cat(cat_shuffled1, 1)
        cat_shuffled1 = self.fusion_ddwc_dwc_1(cat_shuffled1)

        ddwc9x9_shuffled2 = self.ddwc9x9_shuff2(out_shuffled2)
        ddwc9x9_shuffled1_avg2 = ddwc9x9_shuffled2 * out_avg2
        ddwc9x9_shuffled1_max2 = ddwc9x9_shuffled2 * out_max2
        cat_shuffled2 = [ddwc9x9_shuffled1_avg2, ddwc9x9_shuffled1_max2]
        cat_shuffled2 = torch.cat(cat_shuffled2, 1)
        cat_shuffled2 = self.fusion_ddwc_dwc_2(cat_shuffled2)

        ddwc9x9_shuffled3 = self.ddwc9x9_shuff3(out_shuffled3)
        ddwc9x9_shuffled1_avg3 = ddwc9x9_shuffled3 * out_avg3
        ddwc9x9_shuffled1_max3 = ddwc9x9_shuffled3 * out_max3
        cat_shuffled3 = [ddwc9x9_shuffled1_avg3, ddwc9x9_shuffled1_max3]
        cat_shuffled3 = torch.cat(cat_shuffled3, 1)
        cat_shuffled3 = self.fusion_ddwc_dwc_3(cat_shuffled3)


        cat_shuffled = [cat_shuffled1, cat_shuffled2, cat_shuffled3]
...
#        dwc_out = [cat_shuffled, out_dwc_3x3, out_dwc_5x5, out_dwc_7x7]
        dwc_out = [cat_shuffled, out_dwc_7x7]
