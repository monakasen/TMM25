...
class ENLABlock(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, average=True, conv=default_conv, height=None, weight=None):
        ...
        self.dwc_shuff = nn.Conv2d(in_channels=channel, out_channels=channel, kernel_size=9, groups=channel, padding=((9//2)*4), dilation=4)
    ...
    def forward(self, x):
        ...
        # 将索引应用到最后一个维度上进行打乱
        out_shuffled = out_view[..., indices]
        out_shuffled = out_shuffled.view((N, C, H, W))

        dwc_shuffled = self.dwc_shuff(out_shuffled)
        out_dwc_3x3 = self.dwc_3x3(out)
        out_dwc_5x5 = self.dwc_5x5(out)
        out_dwc_7x7 = self.dwc_7x7(out)

        dwc_out = [out_dwc_3x3, out_dwc_5x5, out_dwc_7x7, dwc_shuffled]
...
class NLCUnetShuffle(nn.Module):
    def __init__(
        self,
        nf=64,
        nb=40,
        ...
        super(NLCUnetShuffle, self).__init__()
