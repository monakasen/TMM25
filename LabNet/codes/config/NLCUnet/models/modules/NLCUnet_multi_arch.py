...
class ENLABlock(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, average=True, conv=default_conv, height=None, weight=None):
        ...
        self.w_final = nn.Conv2d(3 * channel, channel, 1)
...
class CRB_Layer(nn.Module):
    def __init__(self, nf1):
        ...
        self.finetune = nn.Conv2d(nf1, nf1, 3, groups=nf1, padding=3 // 2)
...
class Restorer(nn.Module):
    def __init__(
        ...
        self.head_x2 = nn.Conv2d(in_nc, nf, 3, stride=1, padding=1)
        self.head_x4 = nn.Conv2d(in_nc, nf, 3, stride=1, padding=1)

        self.head_fusion = nn.Conv2d(3 * nf, nf, 1, 1)
    ...
    def forward(self, input):
        ...
        # bicubic
        lr_bic_x2 = F.interpolate(input, scale_factor=2, mode='bicubic')
        lr_bic_x2 = torch.clamp(lr_bic_x2, min=0, max=1)
        
        lr_bic_x4 = F.interpolate(input, scale_factor=4, mode='bicubic')
        lr_bic_x4 = torch.clamp(lr_bic_x4, min=0, max=1)

        f_x2 = self.head_x2(lr_bic_x2)
        f_x2 = F.interpolate(f_x2, scale_factor=0.5, mode='bicubic')

        f_x4 = self.head_x4(lr_bic_x4)
        f_x4 = F.interpolate(f_x4, scale_factor=0.25, mode='bicubic')
        
        f_ori = self.head(input)

        f = [f_ori, f_x2, f_x4]
        f = torch.cat(f, 1)        
        f = self.head_fusion(f)
        ...
        out = self.upscale(f) + lr_bic_x4

        return out  # torch.clamp(out, min=self.min, max=self.max)


class NLCUnet_multi(nn.Module):
    def __init__(
        self,
        nf=64,
        nb=40,
        ...
        super(NLCUnet_multi, self).__init__()
