...
class STEA(nn.Module):
    def __init__(self, channel=128, reduction=2, ksize=1, scale=3, stride=1, average=True, conv=default_conv, height=None, weight=None):
        super(STEA, self).__init__()
...
class Restorer(nn.Module):
    def __init__(
        ...
        # encoder block 1
        encoder_block1 = [Block(nf) for _ in range(6)]
        ...
        # encoder block 2
        encoder_block2 = [Block(nf * 2) for _ in range(8)]
        ...
        # latent block
        latent_block = [Block(nf * 4) for _ in range(10)]
        ...
        # decoder block 1
        decoder_block1 = [Block(nf * 2) for _ in range(6)]
        ...
        # decoder block 2
        decoder_block2 = [Block(nf * 2) for _ in range(8)]
        self.decoder_block2 = nn.Sequential(*decoder_block2)

        # refine block
        refine_block = [Block(nf * 2) for _ in range(8)]
    ...
    def forward(self, input):
        ...
        # bicubic
        upsample = transforms.Resize((input.shape[2] * self.scale, input.shape[3] * self.scale), interpolation=transforms.InterpolationMode.BICUBIC)
